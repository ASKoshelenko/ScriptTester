...
const setupCommands = {
  ...
  '4_search_scripts_by_user.sh': `mkdir -p ${config.sandboxDir}/test_dir && touch ${config.sandboxDir}/test_dir/script1.sh && chown $(whoami) ${config.sandboxDir}/test_dir/script1.sh && chmod +x ${config.sandboxDir}/test_dir/script1.sh && echo "Sandbox setup for testing 4_search_scripts_by_user.sh completed"`,
  ...
  '7_find_symbolic_links.sh': `mkdir -p ${config.sandboxDir}/test_dir && ln -s ${config.sandboxDir}/test_dir/file1 ${config.sandboxDir}/test_dir/link1 && echo "Sandbox setup for testing 7_find_symbolic_links.sh completed"`,
  '8_find_hard_links.sh': `mkdir -p ${config.sandboxDir}/test_dir && touch ${config.sandboxDir}/test_dir/file1 && ln ${config.sandboxDir}/test_dir/file1 ${config.sandboxDir}/test_dir/hardlink1 && echo "Sandbox setup for testing 8_find_hard_links.sh completed"`,
  ...
  '11_delete_file_with_links.sh': `mkdir -p ${config.sandboxDir}/test_dir && touch ${config.sandboxDir}/test_dir/file1 && ln -s ${config.sandboxDir}/test_dir/file1 ${config.sandboxDir}/test_dir/link1 && echo "Sandbox setup for testing 11_delete_file_with_links.sh completed"`,
  '12_recursive_change_permissions.sh': `mkdir -p ${config.sandboxDir}/test_dir && touch ${config.sandboxDir}/test_dir/file1 && chmod 644 ${config.sandboxDir}/test_dir/file1 && echo "Sandbox setup for testing 12_recursive_change_permissions.sh completed"`,
  '13_compare_directories.sh': `mkdir -p ${config.sandboxDir}/dir1 && mkdir -p ${config.sandboxDir}/dir2 && echo "File in dir1" > ${config.sandboxDir}/dir1/file1.txt && echo "File in dir2" > ${config.sandboxDir}/dir2/file1.txt && echo "Sandbox setup for testing 13_compare_directories.sh completed"`,
  '14_get_mac_addresses.sh': `echo "Sandbox setup was not required for testing 14_get_mac_addresses.sh"`,
  '15_list_authorized_users.sh': `echo "Sandbox setup was not required for testing 15_list_authorized_users.sh"`,
  '16_list_active_network_connections.sh': `echo "Sandbox setup was not required for testing 16_list_active_network_connections.sh"`,
  '17_reassign_symbolic_link.sh': `mkdir -p ${config.sandboxDir}/test_dir && touch ${config.sandboxDir}/test_dir/file1 && ln -s ${config.sandboxDir}/test_dir/file1 ${config.sandboxDir}/test_dir/link1 && echo "Sandbox setup for testing 17_reassign_symbolic_link.sh completed"`,
  '18_create_symbolic_links.sh': `mkdir -p ${config.sandboxDir}/test_dir && echo "/path/to/file1" > ${config.sandboxDir}/file_list && echo "Sandbox setup for testing 18_create_symbolic_links.sh completed"`,
  '19_copy_directory_with_links.sh': `mkdir -p ${config.sandboxDir}/test_dir && touch ${config.sandboxDir}/test_dir/file1 && ln -s ${config.sandboxDir}/test_dir/file1 ${config.sandboxDir}/test_dir/link1 && echo "Sandbox setup for testing 19_copy_directory_with_links.sh completed"`,
  '20_copy_directory_with_symlinks.sh': `mkdir -p ${config.sandboxDir}/test_dir && touch ${config.sandboxDir}/test_dir/file1 && ln -s ${config.sandboxDir}/test_dir/file1 ${config.sandboxDir}/test_dir/link1 && echo "Sandbox setup for testing 20_copy_directory_with_symlinks.sh completed"`,
  '21_copy_with_attributes.sh': `mkdir -p ${config.sandboxDir}/test_dir && touch ${config.sandboxDir}/test_dir/file1 && echo "Sandbox setup for testing 21_copy_with_attributes.sh completed"`,
  '22_convert_relative_to_direct_links.sh': `mkdir -p ${config.sandboxDir}/test_dir && touch ${config.sandboxDir}/test_dir/file1 && ln -s file1 ${config.sandboxDir}/test_dir/link1 && echo "Sandbox setup for testing 22_convert_relative_to_direct_links.sh completed"`,
  '23_convert_direct_to_relative_links.sh': `mkdir -p ${config.sandboxDir}/test_dir && touch ${config.sandboxDir}/test_dir/file1 && ln -s ${config.sandboxDir}/test_dir/file1 ${config.sandboxDir}/test_dir/link1 && echo "Sandbox setup for testing 23_convert_direct_to_relative_links.sh completed"`,
  '24_find_and_delete_broken_links.sh': `mkdir -p ${config.sandboxDir}/test_dir && ln -s ${config.sandboxDir}/test_dir/non_existent ${config.sandboxDir}/test_dir/broken_link && echo "Sandbox setup for testing 24_find_and_delete_broken_links.sh completed"`,
  '25_unpack_archive.sh': `mkdir -p ${config.sandboxDir}/test_dir && echo "Test file" > ${config.sandboxDir}/test_dir/file1 && tar -cvf ${config.sandboxDir}/test_archive.tar ${config.sandboxDir}/test_dir && echo "Sandbox setup for testing 25_unpack_archive.sh completed"`,
  '26_pack_directory_with_attributes.sh': `mkdir -p ${config.sandboxDir}/test_dir && touch ${config.sandboxDir}/test_dir/file1 && echo "Sandbox setup for testing 26_pack_directory_with_attributes.sh completed"`,
  '27_copy_directory_structure.sh': `mkdir -p ${config.sandboxDir}/test_dir/subdir1 && mkdir -p ${config.sandboxDir}/test_dir/subdir2 && echo "Sandbox setup for testing 27_copy_directory_structure.sh completed"`,
  '28_list_users_alphabetically.sh': `echo "Sandbox setup was not required for testing 28_list_users_alphabetically.sh"`,
  '29_list_system_users_sorted_by_id.sh': `echo "Sandbox setup was not required for testing 29_list_system_users_sorted_by_id.sh"`,
  '30_list_users_sorted_by_reverse_id.sh': `echo "Sandbox setup was not required for testing 30_list_users_sorted_by_reverse_id.sh"`,
  '31_list_users_without_authorization_rights.sh': `echo "Sandbox setup was not required for testing 31_list_users_without_authorization_rights.sh"`,
  '32_list_users_with_or_without_terminal.sh': `echo "Sandbox setup was not required for testing 32_list_users_with_or_without_terminal.sh"`,
  '33_download_all_links_from_page.sh': `echo "Sandbox setup was not required for testing 33_download_all_links_from_page.sh"`,
  '34_stop_long_running_processes.sh': `echo "Sandbox setup was not required for testing 34_stop_long_running_processes.sh"`,
  ...
  '36_find_ip_address.sh': `echo "Sandbox setup was not required for testing 36_find_ip_address.sh"`,
  ...
  '39_get_raised_interfaces_ips.sh': `echo "Sandbox setup was not required for testing 39_get_raised_interfaces_ips.sh"`,
  '40_get_subdomains_from_ssl.sh': `echo "Sandbox setup was not required for testing 40_get_subdomains_from_ssl.sh"`,
  '41_extract_path_name_extension.sh': `echo "Sandbox setup was not required for testing 41_extract_path_name_extension.sh"`,
  '42_delete_files_by_size_and_pattern.sh': `mkdir -p ${config.sandboxDir}/test_dir && dd if=/dev/zero of=${config.sandboxDir}/test_dir/file1.txt bs=1 count=22 && touch ${config.sandboxDir}/test_dir/file2.txt && echo "Sandbox setup for testing 42_delete_files_by_size_and_pattern.sh completed"`,
  '43_create_files_with_identifiers.sh': `mkdir -p ${config.sandboxDir}/test_dir && echo "${config.sandboxDir}/test_dir/file1 1234" > ${config.sandboxDir}/file_list && echo "${config.sandboxDir}/test_dir/file2 5678" >> ${config.sandboxDir}/file_list && echo "Sandbox setup for testing 43_create_files_with_identifiers.sh completed"`
};
...
const commands = {
  ...
  '9_find_names_by_inode.sh': `ls -i ${config.sandboxDir}/test_dir/file1 | cut -d' ' -f1 | xargs find ${config.sandboxDir}/test_dir -inum && echo "Task completed"`,
  '10_find_names_by_inode_multiple_partitions.sh': `ls -i ${config.sandboxDir}/test_dir/file1 | cut -d' ' -f1 | xargs find ${config.sandboxDir}/test_dir -xdev -inum && echo "Task completed"`,
  ...
  '12_recursive_change_permissions.sh': `find ${config.sandboxDir}/test_dir -type f -perm 644 -exec chmod 640 {} \\; && echo "Task completed"`,
  '13_compare_directories.sh': `diff -rN -u5 ${config.sandboxDir}/dir1/ ${config.sandboxDir}/dir2/ && echo "Task completed"`,
  ...
  '18_create_symbolic_links.sh': `mkdir -p ${config.sandboxDir}/sym_dir && cat ${config.sandboxDir}/file_list | xargs -I{} sh -c 'ln -sbrf {} ${config.sandboxDir}/sym_dir/link_to_$(basename {})' && echo "Task completed"`,
  ...
  '21_copy_with_attributes.sh': `cp -rp ${config.sandboxDir}/test_dir ${config.sandboxDir}/dir4/ && echo "Task completed"`,
  '22_convert_relative_to_direct_links.sh': `find ${config.sandboxDir}/test_dir -type l | xargs -I{} sh -c 'ln -sf $(realpath {}) {}' && echo "Task completed"`,
  '23_convert_direct_to_relative_links.sh': `find ${config.sandboxDir}/test_dir -type l | xargs -I{} sh -c 'ln -srf $(readlink {}) {}' && echo "Task completed"`,
  ...
  '35_delete_orphaned_jpeg_files.sh': `find ${config.sandboxDir}/test_dir -type f -name '*.jpeg' | xargs -I{} sh -c 'pname="$(dirname {})/$(basename -s .jpeg {})"; [ ! -e \${pname}.txt ] && rm \${pname}.jpeg && echo "Deleted: \${pname}.jpeg"' && echo "Task completed"`,
  ...
  '42_delete_files_by_size_and_pattern.sh': `find ${config.sandboxDir}/test_dir -type f -size 22c -name '*.txt' -delete && echo "Task completed"`,
  '43_create_files_with_identifiers.sh': `xargs -a ${config.sandboxDir}/file_list -n2 sh -c 'mkdir -p $(dirname $1) && echo $2 > $1' && echo "Task completed"`
};
