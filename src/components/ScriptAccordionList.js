...
const scripts = {
  ...
  '4_search_scripts_by_user.sh': {
    ...
    command: 'sudo -u root -H find . -type f -executable -exec sh -c \'head -c2 "$1" | grep -q "#!" && echo "$1"\' _ {} \\; && echo "Task completed"'
  },
  ...
  '7_find_symbolic_links.sh': {
    ...
    command: 'find . -type l -lname *file1.txt && echo "Task completed"'
  },
  ...
  '20_copy_directory_with_symlinks.sh': {
    description: 'Copy a directory, preserving direct and symbolic links.',
  ...
  '21_copy_with_attributes.sh': {
    description: 'Copy all files and directories from the specified directory to a new location, preserving attributes and permissions.',
  ...
  '22_convert_relative_to_direct_links.sh': {
    description: 'Convert all relative links in the project to direct links.',
  ...
  '23_convert_direct_to_relative_links.sh': {
    description: 'Convert all direct links in the project to relative links, relative to the project directory.',
  ...
  '24_find_and_delete_broken_links.sh': {
    description: 'Find and delete all broken links in the specified directory.',
    command: 'find /home/ask/ubuntu-scripts-tester/sandbox/test_dir -xtype l -delete && echo "Task completed"'
  },
  '25_unpack_archive.sh': {
    description: 'Unpack a tar, gz, bz2, lz, lzma, xz, Z archive to the specified location.',
  ...
  '26_pack_directory_with_attributes.sh': {
    description: 'Pack a directory structure with files, preserving all attributes and permissions.',
  ...
  '27_copy_directory_structure.sh': {
    description: 'Recursively copy the directory structure from the specified directory.',
  ...
  '31_list_users_without_authorization_rights.sh': {
    description: 'List all users who do not have login rights.',
  ...
  '33_download_all_links_from_page.sh': {
    description: 'Download all href links from a webpage.',
  ...
  '34_stop_long_running_processes.sh': {
    description: 'Stop processes running for more than 5 days.',
  ...
  '35_delete_orphaned_jpeg_files.sh': {
    description: 'Delete all .jpeg files without corresponding .txt files.',
    command: 'find /home/ask/ubuntu-scripts-tester/sandbox/test_dir -type f -name \'*.jpeg\' | xargs -I{} sh -c \'pname="$(dirname {})/$(basename -s .jpeg {})"; [ ! -e ${pname}.txt ] && rm ${pname}.jpeg\' && echo "Task completed"'
  },
  '36_find_ip_address.sh': {
    description: 'Find your IP address using the command line.',
    command: 'ip -o addr | grep inet | cut -d \' \' -f2,7 | tail -n +3 && echo "Task completed"'
  },
  '37_get_all_ip_addresses_from_file.sh': {
    description: 'Get all IP addresses from a text file.',
  ...
  '38_find_active_hosts.sh': {
    description: 'Find all active hosts in the specified network or list of IPs.',
  ...
  '39_get_raised_interfaces_ips.sh': {
    description: 'Get the IP addresses of all raised interfaces.',
  ...
  '40_get_subdomains_from_ssl.sh': {
    description: 'Get all subdomains from an SSL certificate.',
  ...
  '41_extract_path_name_extension.sh': {
    description: 'Extract the path, name, and extension of a file from a string in Bash.',
  ...
  '42_delete_files_by_size_and_pattern.sh': {
    description: 'Delete files of the specified size and name pattern from the specified directory.',
  ...
  '43_create_files_with_identifiers.sh': {
    description: 'Create files and write corresponding identifiers to them from a list.',
    command: 'xargs -a /home/ask/ubuntu-scripts-tester/sandbox/hash_file_list -n2 sh -c \'mkdir -p $(dirname $1) && echo $2 > $1\' && echo "Task completed"'
  }
};
